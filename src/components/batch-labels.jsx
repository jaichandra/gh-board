  ...
  render() {
    ...
    if (isEditing) {
      ...
      if (repoInfos.length === 0) {
        ...
        details = (
          <a target='_window' href={getRepoUrl() + `/${repoOwner}/${repoName}/labels`}>{repoName}</a>
        );
        ...
        details = (
          <span>
            <a target='_window' href={getRepoUrl() + `/${repoOwner}/${repoName}/labels`}>{repoName}</a>
            <span> & </span>
            <a target='_window' href={getRepoUrl() + `/${repoOwner2}/${repoName2}/labels`}>{repoName2}</a>
          </span>
        );
      } else {
        const [repoOwner, repoName] = repoInfos[0].split('/');
        const popover = (
          <BS.Popover id={`repos-with-${label.name}`} title='Repositories that have this label'>
            {repoInfos.slice(1).sort().map((repoInfo) => {
              ...
              return (
                <p key={repoInfo}>
                  <a target='_window' href={getRepoUrl() + `/${repoOwner1}/${repoName1}/labels`}>{repoOwner1}/{repoName1}</a>
                </p>
              )
            })}
          </BS.Popover>
...
const BatchLabelsShell = React.createClass({
  ...
  renderLoaded(repoInfosWithLabels) {
    ...
    const uniquePanels = repoInfosWithUniqueLabels.map(({repoOwner, repoName, uniqueLabels}) => {
      const header = (
        <span>Labels unique to <a target='_window' href={getRepoUrl() + `/${repoOwner}/${repoName}/labels`}>{repoOwner}/{repoName}</a></span>
      )
      return (
